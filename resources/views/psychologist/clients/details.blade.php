...
<div class="flex flex-col flex-1 gap-6 min-w-0 h-full overflow-y-auto pr-2 pb-20 scroll-smooth">
    ...
    <div class="flex items-center justify-between bg-white p-6 rounded-md border-grey-border border">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-primary font-bold text-lg">Client Details</h1>
                <h5 class="text-captiondark text-sm">{{ $client->full_name }}'s information and session history</h5>
            </div>
        </div>
    </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="bg-gradient-to-br from-primary/10 via-blue-50 to-purple-50 pt-8 pb-6 px-6">
                    <div class="text-center">
                        ...
                        <div class="inline-flex items-center gap-1.5 mt-2 px-3 py-1 bg-white/80 backdrop-blur-sm rounded-full text-sm text-gray-600">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="space-y-3 pl-0.5">
                            <div class="flex items-start gap-3 group">
                                <svg class="w-4 h-4 text-gray-400 mt-0.5 group-hover:text-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="flex items-start gap-3 group">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-gray-800 font-medium">{{ $client->date_of_birth->format('d M Y') }} <span class="text-gray-500">({{ $client->date_of_birth->age }} years)</span></p>
                                </div>
                            </div>
                            <div class="flex items-start gap-3 group">
                                <svg class="w-4 h-4 text-gray-400 mt-0.5 group-hover:text-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-gray-800 font-medium">{{ ucfirst($client->gender) }}</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-3 group">
                                <svg class="w-4 h-4 text-gray-400 mt-0.5 group-hover:text-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="flex-1">
                                    <p class="text-xs text-gray-500 mb-0.5">Preferred Language</p>
                                    <p class="text-sm text-gray-800 font-medium">{{ $client->preferred_language == 'en' ? 'English' : 'Indonesian' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <h4 class="font-semibold text-gray-800">Session Overview</h4>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div class="relative overflow-hidden bg-gradient-to-br from-blue-50 to-blue-100/50 p-4 rounded-xl border border-blue-200/50 hover:shadow-md transition-all duration-200 group">
                                <div class="absolute top-0 right-0 w-16 h-16 bg-blue-200/30 rounded-full -mr-8 -mt-8 group-hover:scale-110 transition-transform"></div>
                                <div class="relative">
                                    <p class="text-3xl font-bold text-blue-700 mb-1">{{ $appointments->where('status', 'completed')->count() }}</p>
                                    <p class="text-xs text-blue-600 font-medium">Completed Sessions</p>
                                </div>
                            </div>
                            <div class="relative overflow-hidden bg-gradient-to-br from-green-50 to-green-100/50 p-4 rounded-xl border border-green-200/50 hover:shadow-md transition-all duration-200 group">
                                <div class="absolute top-0 right-0 w-16 h-16 bg-green-200/30 rounded-full -mr-8 -mt-8 group-hover:scale-110 transition-transform"></div>
                                <div class="relative">
                                    <p class="text-3xl font-bold text-green-700 mb-1">{{ $appointments->where('status', 'confirmed')->count() }}</p>
                                    <p class="text-xs text-green-600 font-medium">Upcoming Sessions</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-white to-gray-50">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-gray-600 text-sm mt-1">Complete therapy timeline for {{ $client->full_name }}</p>
                        </div>
                        @if($appointments->count() > 0)
                        <span class="px-3 py-1.5 bg-primary/10 text-primary text-xs font-bold rounded-full">
                            {{ $appointments->count() }} {{ $appointments->count() == 1 ? 'Session' : 'Sessions' }}
                        </span>
                        @endif
                    </div>
                </div>
                ...
                <div class="divide-y divide-gray-100">
                    ...
                    <div class="p-6 hover:bg-gradient-to-r hover:from-gray-50 hover:to-blue-50/30 transition-all duration-200 group">
                        <div class="flex items-start gap-5">
                            <!-- Date  -->
                            <div class="flex-shrink-0">
                                <div class="text-center bg-gradient-to-br from-primary/10 to-blue-100/50 rounded-xl p-3 border border-primary/20 min-w-[70px] group-hover:shadow-md group-hover:scale-105 transition-all duration-200">
                            ...
                            <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="flex items-center gap-3 text-sm text-gray-600">
                                            ...
                                            <span class="flex items-center gap-1.5 font-medium">
                                                ...
                                                {{ number_format($appointment->consultation_fee, 0, ',', '.') }} IDR
                                            </span>
                                        </div>
                                    </div>
                                    <span class="px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap
                                        {{ $appointment->status === 'confirmed' ? 'bg-blue-100 text-blue-500 border border-blue-200' :
                                        ($appointment->status === 'completed' ? 'bg-green-100 text-green-500 border border-green-200' :
                                ...
                                <div class="mt-3 p-4 bg-gradient-to-br from-blue-50 to-indigo-50/50 rounded-lg border border-blue-200/50 group/notes hover:shadow-sm transition-all duration-200">
                                    <div class="flex items-center justify-between mb-2">
                                        <span class="text-sm font-semibold text-blue-600 flex items-center gap-1.5">
                                    ...
                                    <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{{ $appointment->notes }}</p>
                                </div>
                                @else
                                <div class="mt-3">
                                    <button type="button"
                                            onclick="addNotes({{ $appointment->id }})"
                ...
                <div class="p-16 text-center">
                    ...
                    <h4 class="text-lg font-semibold text-gray-700 mb-2">No Sessions Yet</h4>
                </div>
                @endif
            </div>
        </div>
            ...
            <form id="notesForm" onsubmit="saveNotes(event)">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" id="sessionNotes" rows="6"
                              ...
                              placeholder="Document observations, progress, key insights, treatment plans, and recommendations..."
                              required></textarea>
...
function editNotes(appointmentId, existingNotes) {
    document.getElementById('modalTitle').textContent = 'Edit Session Notes';
    document.getElementById('modalAppointmentId').value = appointmentId;
