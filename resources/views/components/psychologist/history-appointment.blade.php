...
<div class="bg-white p-6 flex flex-col gap-6 rounded-md border-grey-border border">
    <div class="flex flex-1 items-start">
        <h3 class="font-bold">Session History</h3>
    </div>
    ...
    @forelse($appointments as $item)
        ...
        <div class="bg-white p-4 rounded-xl border border-grey-border hover:shadow-md transition flex items-center justify-between group cursor-pointer"
            ...
            <div class="flex items-center gap-4">
                <div class="relative">
                    <div class="w-10 h-10 rounded-full {{ $statusConfig['icon'] }} flex items-center justify-center group-hover:bg-green-500 group-hover:text-white transition">
                ...
                <div>
                    <p class="font-bold text-gray-800 text-sm">
                        {{ $item->user->full_name ?? 'Unknown Patient' }}
                    </p>
                    <p class="text-xs text-gray-500">
                        ...
                        @if($item->user->gender || $item->user->date_of_birth)
                            ...
                            <span class="text-gray-400">
                                @if($item->user->gender)
                                    {{ $item->user->gender == 'female' ? 'Female' : 'Male' }}
                                @endif
                                @if($item->user->date_of_birth)
                                    • {{ \Carbon\Carbon::parse($item->user->date_of_birth)->age }} years
                                @endif
                            </span>
                        @endif
                    </p>
            ...
            <div class="flex items-center gap-3">
                <span class="px-3 py-1 rounded-full text-[10px] font-bold {{ $statusConfig['badge'] }} uppercase tracking-wide">
                    {{ $statusText }}
                </span>
    ...
    @empty
        <div class="bg-white p-6 text-center rounded-md border-grey-border border flex flex-col gap-6">
            <p class="text-gray-500">No session history found</p>
            <p class="text-sm text-gray-400">Completed and cancelled sessions will appear here</p>
        </div>
    @endforelse
</div>
