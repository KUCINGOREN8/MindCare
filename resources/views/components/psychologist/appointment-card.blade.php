    ...
    <div class="p-4 flex flex-col gap-4 rounded-md border border-grey-border bg-white hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col gap-0">
                    ...
                    <p class="text-gray-600 text-sm">
                        {{-- TRANSLATE GENDER & AGE --}}
                        @if ($patientAge)
                            {{ $appointment->user->gender == 'female' ? __('psychologist_appointment.female') : __('psychologist_appointment.male') }},
                            {{ $patientAge }} {{ __('psychologist_appointment.years') }}
                        @else
                            {{ $appointment->user->gender == 'female' ? __('psychologist_appointment.female') : __('psychologist_appointment.male') }}
                            {{ __('psychologist_appointment.patient_suffix') }}
                        @endif
            ...
            <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide {{ $statusClass }}">
                {{ __('psychologist_appointment.status_' . $appointment->status) }}
            </span>
        </div>
        ...
        <div class="flex gap-2">
            @if ($appointment->is_session_available && $appointment->status === 'confirmed')
                <a href="{{ route('psychologist.appointments.chat.session', $appointment->id) }}"
                    ...
                    {{ __('psychologist_appointment.btn_join') }}
                </a>
            @elseif($appointment->status === 'confirmed')
                <button
                    class="flex-1 px-4 py-2 bg-gray-100 text-gray-500 rounded-md text-center text-sm font-medium cursor-not-allowed">
                    @if (now() < $appointment->start_date_time)
                        {{ __('psychologist_appointment.btn_starts_at', ['time' => \Carbon\Carbon::parse($appointment->start_time)->format('H:i')]) }}
                    @else
                        {{ __('psychologist_appointment.btn_ended') }}
                    @endif
                </button>
            @endif
            ...
            <a href="{{ route('psychologist.clients.details', $appointment->user_id) }}"
                ...
                {{ __('psychologist_appointment.btn_profile') }}
            </a>

            <button
                class="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-50 text-sm font-medium transition flex items-center justify-center gap-1.5"
                onclick="openSessionNotesModal({{ $appointment->id }}, '{{ addslashes($appointment->notes ?? '') }}', '{{ $appointment->user->full_name }}', '{{ \Carbon\Carbon::parse($appointment->date)->translatedFormat('d M Y') }}')"
                title="{{ __('psychologist_appointment.tooltip_notes') }}">
        ...
        @if ($appointment->status === 'pending_payment')
            <div class="text-xs text-yellow-600 bg-yellow-50 p-2 rounded border border-yellow-100">
                {{ __('psychologist_appointment.payment_warning') }}
            </div>
        @endif
    </div>
@endif
        ...
        <div class="p-6">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-xl font-bold text-gray-900 flex items-center gap-2" id="modalTitle">
                    <span id="modalTitleText">{{ __('psychologist_appointment.modal_title') }}</span>
                </h3>
            ...
            <form id="sessionNotesForm" onsubmit="saveSessionNotes(event)">
                ...
                <div class="mb-6">
                    <label
                        class="block text-sm font-semibold text-gray-700 mb-2">{{ __('psychologist_appointment.modal_label') }}</label>
                    <textarea name="notes" id="sessionNotesTextarea" rows="6"
                        ...
                        placeholder="{{ __('psychologist_appointment.modal_placeholder') }}" required></textarea>
                    <p class="mt-2 text-xs text-gray-500">{{ __('psychologist_appointment.modal_help') }}</p>
                </div>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="closeSessionNotesModal()"
                        ...
                        {{ __('psychologist_appointment.btn_cancel') }}
                    </button>
                    <button type="submit"
                        ...
                        {{ __('psychologist_appointment.btn_save') }}
                    </button>
                </div>
            </form>
        </div>
...
<script>
    function openSessionNotesModal(appointmentId, existingNotes, patientName, sessionDate) {
        ...
        // Judul modal pakai teks default dari lang PHP
        document.getElementById('modalTitleText').textContent = "{{ __('psychologist_appointment.modal_title') }}";
    ...
    function saveSessionNotes(event) {
        ...
        const originalText = submitBtn.innerHTML; // Simpan innerHTML untuk icon

        submitBtn.disabled = true;
        submitBtn.textContent = "{{ __('psychologist_appointment.js_saving') }}";
        ...
        fetch("{{ route('psychologist.notes.session.store') }}", {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert(data.message || "{{ __('psychologist_appointment.js_failed') }}");
                }
            })
            .catch(err => {
