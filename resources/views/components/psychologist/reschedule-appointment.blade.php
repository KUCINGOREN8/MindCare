...
<div class="bg-white p-6 flex flex-col gap-6 rounded-md border-grey-border border">
    ...
    @forelse($appointments as $appointment)
        @php
            $originalDate = \Carbon\Carbon::parse($appointment->date)->format('d M Y');
        ...
        <div class="bg-yellow-50 p-6 rounded-2xl border border-yellow-200">
            <div class="flex items-center gap-4 mb-4">
                <div class="w-12 h-12 rounded-full bg-yellow-100 border-2 border-yellow-300 flex items-center justify-center text-yellow-600 font-bold">
                    {{ substr($appointment->user->full_name ?? 'P', 0, 1) }}
                </div>
                <div>
                    <h4 class="font-bold text-gray-900">{{ $appointment->user->full_name ?? 'Patient' }}</h4>
                    <p class="text-sm text-gray-600">
                        Session ID: #{{ $appointment->id }}
                        @if($appointment->user->date_of_birth)
                            • {{ \Carbon\Carbon::parse($appointment->user->date_of_birth)->age }} years
                        @endif
                    </p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div class="bg-white p-4 rounded-xl border border-gray-200">
                    <p class="text-xs text-gray-500 mb-2 font-medium">CURRENT SCHEDULE</p>
                ...
                <div class="bg-yellow-50 p-4 rounded-xl border border-yellow-300">
                    <p class="text-xs text-yellow-600 mb-2 font-medium">REQUESTED TIME</p>
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        <span class="text-sm font-medium text-yellow-700">{{ $requestedDate }}, {{ $requestedTime }}</span>
                    </div>
                </div>
            </div>
            ...
            @if($appointment->reschedule_reason)
                <div class="mb-4">
                    <p class="text-xs text-gray-500 mb-1 font-medium">PATIENT'S REASON</p>
            ...
            <div class="text-xs text-gray-500 bg-white p-3 rounded-lg border border-gray-200">
                ⚠️ <span class="font-medium">Action Required:</span> This reschedule request is pending your review.
                The accept/reject functionality will be implemented in the next phase.
            </div>
        </div>
    @empty
        <div class="bg-white p-6 text-center rounded-md border-grey-border border flex flex-col gap-6">
            <p class="text-gray-500">No pending reschedule requests</p>
            <p class="text-sm text-gray-400">Patient reschedule requests will appear here</p>
        </div>
    @endforelse
</div>
