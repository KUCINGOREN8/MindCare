...
@section('content')
    @php
        ...
        // TRANSLATE ARRAY DEFINITION
        $tabs = [
            'profile' => [
                'name' => __('settings.tab_profile_name'),
                'title' => __('settings.tab_profile_title'),
                'subtitle' => __('settings.tab_profile_desc'),
                'icon' => 'user.svg',
                'roles' => ['patient', 'psychologist', 'admin'],
            ],
            'professional' => [
                'name' => __('settings.tab_professional_name'),
                'title' => __('settings.tab_professional_title'),
                'subtitle' => __('settings.tab_professional_desc'),
                'icon' => 'work.svg',
                'roles' => ['psychologist'],
            ],
            'schedule' => [
                'name' => __('settings.tab_schedule_name'),
                'title' => __('settings.tab_schedule_title'),
                'subtitle' => __('settings.tab_schedule_desc'),
                'icon' => 'calendar.svg',
                'roles' => ['psychologist'],
            ],
            'privacy' => [
                'name' => __('settings.tab_privacy_name'),
                'title' => __('settings.tab_privacy_title'),
                'subtitle' => __('settings.tab_privacy_desc'),
                'icon' => 'password.svg',
                'roles' => ['patient', 'psychologist', 'admin'],
            ],
            'preferences' => [
                'name' => __('settings.tab_preferences_name'),
                'title' => __('settings.tab_preferences_title'),
                'subtitle' => __('settings.tab_preferences_desc'),
                'icon' => 'language.svg',
                'roles' => ['patient', 'psychologist', 'admin'],
            ],
        ];
            ...
            <div class="flex flex-col bg-white p-4 sm:p-6 rounded-md border-grey-border border justify-between gap-6">
                ...
                <nav class="tabs overflow-x-auto space-x-1">
                    @foreach ($availableTabs as $tabKey => $tabConfig)
                        ...
                        {{ $tab === $tabKey ? 'bg-[#00C3B3] text-white' : 'hover:bg-[#00C3B3]/10 hover:text-[#00C3B3] text-[#4D4D4E]' }}">
                            ...
                            {{-- ICONS LOGIC (Tidak Berubah) --}}
                            @if ($tabConfig['icon'] === 'user.svg')
                                {!! str_replace(
                                    '<svg ',
                                    '<svg class="w-5 h-5 ' .
                                        ($tab === $tabKey ? 'text-white' : 'text-[#4D4D4E] group-hover:text-[#00C3B3]') .
                                        '" fill="currentColor" ',
                ...
                <div class="mt-3">
                    ...
                    @if ($tab === 'professional' && $user->isPsychologist())
                        @php $psychologist = $user->psychologist; @endphp
                        @include('settings.tabs.professional', [
                            'user' => $user,
                    ...
                    @if ($tab === 'schedule' && $user->isPsychologist())
                        @php $psychologist = $user->psychologist; @endphp
                        @include('settings.tabs.schedule', [
                            'user' => $user,
                            'psychologist' => $psychologist,
                        ])
                    @endif
    ...
    <script>
        // DEFINE TRANSLATION OBJECT FOR JS
        const LANG = {
            btnEdit: "{{ __('settings.btn_edit') }}",
            btnConfirm: "{{ __('settings.btn_confirm') }}",
            available: "{{ __('settings.js_available') }}",
            notAvailable: "{{ __('settings.js_not_available') }}",
            showPass: "{{ __('settings.js_show_pass') }}",
            hidePass: "{{ __('settings.js_hide_pass') }}",
            errRequired: "{{ __('settings.err_required') }}",
            errStartTime: "{{ __('settings.err_start_time') }}",
            errEndTime: "{{ __('settings.err_end_time') }}",
            errTimeLogic: "{{ __('settings.err_time_logic') }}"
        };
        ...
        const EditModeManager = {
                ...
                document.querySelectorAll('.password-toggle').forEach(button => {
                    button.addEventListener('click', (e) => {
                        ...
                        if (input.type === 'password') {
                            ...
                            eyeIcon.alt = LANG.hidePass; // Translate Alt
                        } else {
                            input.type = 'password';
                            eyeIcon.src = eyeIcon.getAttribute('data-closed-icon');
                            eyeIcon.alt = LANG.showPass; // Translate Alt
                        }
                    });
                });
            },
                ...
                document.querySelectorAll('.availability-toggle').forEach(toggle => {
                    toggle.addEventListener('change', function() {
                        ...
                        const hiddenInput = document.querySelector(
                            `input[type="hidden"][data-hidden-for="${day}"]`);

                        if (this.checked) {
                            timeInputs.classList.remove('hidden');
                            if (label) label.textContent = LANG.available; // Translate Status
                            ...
                            if (label) label.textContent = LANG.notAvailable; // Translate Status
            ...
            toggleEditMode(formId) {
                ...
                if (!state.isEditMode) {
                    ...
                    if (editButton) {
                        editButton.textContent = LANG.btnConfirm; // Translate Button
            ...
            validateScheduleForm(form) {
                ...
                checkedToggles.forEach(toggle => {
                    ...
                    if (startTime && !startTime.value.trim()) {
                        isValid = false;
                        this.showValidationError(startTime, LANG.errStartTime); // Translate Error
                    }

                    if (endTime && !endTime.value.trim()) {
                        isValid = false;
                        this.showValidationError(endTime, LANG.errEndTime); // Translate Error
                    }

                    if (startTime && endTime && startTime.value && endTime.value) {
                        if (startTime.value >= endTime.value) {
                            isValid = false;
                            this.showValidationError(endTime, LANG.errTimeLogic); // Translate Error
                        }
                    }
                });
            ...
            validateRequiredFields(form) {
                ...
                requiredInputs.forEach(input => {
                    if (!input.value.trim()) {
                        isValid = false;
                        this.showValidationError(input, LANG.errRequired); // Translate Error
                    }
                });
            ...
            cancelEdit(formId) {
                ...
                if (state.isPasswordForm) {
                    const passwordInputs = form.querySelectorAll(
                        '[type="password"], [type="text"][data-password-field]');
                    ...
                    eyeIcons.forEach(icon => {
                        ...
                        icon.alt = LANG.showPass; // Translate Alt
                    });
                } else {
                ...
                if (editButton) {
                    editButton.textContent = LANG.btnEdit; // Translate Button
                        ...
                        if (input.type === 'checkbox' || input.type === 'radio') {
                            ...
                            if (input.classList.contains('availability-toggle')) {
                                ...
                                if (originalValue === 'true') {
                                    ...
                                    if (label) label.textContent = LANG.available; // Translate
                                } else {
                                    if (timeInputs) timeInputs.classList.add('hidden');
                                    if (label) label.textContent = LANG.notAvailable; // Translate
                                }
                            }
