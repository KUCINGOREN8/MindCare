...
<div class="absolute inset-0 flex bg-gray-50">
    ...
    <div class="flex-1 flex flex-col h-full">
                                        ...
                                        @if($message['attachment_url'] ?? false)
                                            ...
                                            @if(in_array($fileExt, $imageExts))
                                                <div class="mb-2">
                                                    ...
                                                    <p class="text-xs {{ $message['sender_id'] == auth()->id() ? 'text-white/80' : 'text-gray-500' }} mt-1">
                                                        {{ $fileName }}
                                                    </p>
                                                </div>
                                                @if($message['message'] && !in_array($message['message'], ['🖼️ Image', '📄 Document', '📎 Attachment']))
                                                    <p class="text-sm break-words leading-relaxed {{ $message['sender_id'] == auth()->id() ? 'text-white' : 'text-gray-900' }} mt-2">
                                                        {{ $message['message'] }}
                                                    </p>
                                                @endif
                                            @elseif(in_array($fileExt, $docExts))
                                                ...
                                                @endphp
                                                <div class="mb-2 p-3 {{ $message['sender_id'] == auth()->id() ? 'bg-white/20' : 'bg-gray-50' }} rounded-lg border {{ $message['sender_id'] == auth()->id() ? 'border-white/30' : 'border-gray-200' }} hover:opacity-90 transition">
                                                    <div class="flex items-center gap-3">
                                                        <div class="text-2xl">{{ $icon }}</div>
                                                        <div class="flex-1 min-w-0">
                                                            <p class="text-sm font-medium {{ $message['sender_id'] == auth()->id() ? 'text-white' : 'text-gray-900' }} truncate">
                                                                {{ $fileName }}
                                                            </p>
                                                            <div class="flex items-center gap-2 mt-1">
                                                                ...
                                                                <span class="text-xs {{ $message['sender_id'] == auth()->id() ? 'text-white/60' : 'text-gray-500' }}">•</span>
                                                                <span class="text-xs {{ $message['sender_id'] == auth()->id() ? 'text-white/60' : 'text-gray-500' }} uppercase">
                                                                    {{ $fileExt }}
                                                                </span>
                                                            </div>
                                                        </div>
                                                ...
                                                @endif
                                            @else
                                                <div class="mb-2 p-3 {{ $message['sender_id'] == auth()->id() ? 'bg-white/20' : 'bg-gray-50' }} rounded-lg border {{ $message['sender_id'] == auth()->id() ? 'border-white/30' : 'border-gray-200' }}">
                                                    <div class="flex items-center gap-3">
                                                        <div class="text-xl">📁</div>
                                                        <div class="flex-1">
                                                            <p class="text-sm font-medium {{ $message['sender_id'] == auth()->id() ? 'text-white' : 'text-gray-900' }}">
                                                                {{ $fileName }}
                                                            </p>
                                                            <a href="{{ $message['attachment_url'] }}"
                                                            class="text-xs {{ $message['sender_id'] == auth()->id() ? 'text-white hover:text-white/80' : 'text-primary hover:underline' }}"
                                                            target="_blank"
                                                            download="{{ $fileName }}">
                                                                Download File
                                                            </a>
        ...
        <div class="bg-white border-t border-gray-100 p-4 flex-shrink-0">
            <form id="message-form" method="POST" action="{{ route('chat.send', $conversation) }}" enctype="multipart/form-data" class="max-w-3xl mx-auto">
                @csrf
                <input type="file" id="file-input" name="attachment" class="hidden" accept="image/*,.pdf,.doc,.docx,.txt">

                <div class="flex items-center gap-3">
                    <div class="relative">
                        <button type="button" id="attachment-btn" class="text-gray-500 hover:text-gray-700 p-2.5 rounded-xl hover:bg-gray-100 transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                            </svg>
                        </button>

                        <div id="attachment-dropdown" class="absolute bottom-full left-0 mb-2 hidden bg-white rounded-xl shadow-lg border border-gray-200 p-2 w-48 z-50">
                            <button type="button" onclick="document.getElementById('file-input').click()" class="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg">
                    ...
                    {{-- Emoji --}}
                    <div class="relative">
                        <button type="button" id="emoji-btn" class="text-gray-500 hover:text-gray-700 p-2.5 rounded-xl hover:bg-gray-100 transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <!-- Emoji picker -->
                        <div id="emoji-picker" class="absolute bottom-full left-0 mb-2 hidden bg-white rounded-xl shadow-lg border border-gray-200 p-3 w-64 z-50">
                            <div class="grid grid-cols-8 gap-1 mb-2">
                                @php
                                    $emojis = ['😀', '😂', '🥰', '😎', '😢', '😡', '👍', '👏', '❤️', '🔥', '🎉', '🤔', '👀', '💯', '🙏', '😴'];
                                @endphp
                                @foreach($emojis as $emoji)
                                    <button type="button" class="emoji-btn text-lg hover:bg-gray-100 rounded p-1" data-emoji="{{ $emoji }}">
                                        {{ $emoji }}
                                    </button>
                                @endforeach
                            </div>
                    ...
                    <div class="flex-1 relative">
                        ...
                        <div id="file-preview" class="hidden mt-2 p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div class="flex items-center justify-between">
                                ...
                                <button type="button" onclick="removeFile()" class="text-gray-500 hover:text-red-500">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        messageForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (!message && file) {
                    ...
                    if (isImage) {
                        formData.set('message', '🖼️ Image');
                    } else if (isDoc) {
                        formData.set('message', '📄 Document');
                    } else {
                        formData.set('message', '📎 Attachment');
                    }
                }
        ...
        if (messageData.attachment_url) {
            ...
            if (imageExts.includes(fileExt)) {
                messageContent = `
                    ...
                    ${messageData.message && !['🖼️ Image', '📄 Document', '📎 Attachment'].includes(messageData.message) ?
                        `<p class="text-sm break-words leading-relaxed mt-2">${messageData.message}</p>` : ''}
                `;
                    ...
                    <div class="flex items-start gap-2">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-2 mt-1 ${isSent ? 'justify-end' : 'justify-start'}">
                                ...
                                ${isSent ? `
                                    <span class="text-xs ${messageData.is_read ? 'text-primary' : 'text-gray-400'}">
                                        ${messageData.is_read ? '✓ Read' : '✓ Sent'}
                                    </span>
                                ` : ''}
                            </div>
                        </div>
    ...
    function updateSidebarConversation(messageData) {
        ...
        if (conversationItem) {
            ...
            if (lastMessageEl) {
                let preview = messageData.message;
                if (messageData.attachment_url) {
                    const fileExt = (messageData.attachment_name || '').split('.').pop().toLowerCase();
                ...
                lastMessageEl.textContent = preview.length > 25
    ...
    @if(!empty($messages) && is_array($lastMessage = end($messages)) && isset($lastMessage['sender_id']) && $lastMessage['sender_id'] != auth()->id())
    @php
