            ...
            @if ($existingReview)
                <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-6 mb-8">
                    <div class="flex items-center">
                        ...
                        <p class="text-yellow-800 font-medium">{{ __('rate_session.already_reviewed_title') }}</p>
                    </div>
                    <p class="text-yellow-700 mt-2">
                        {{ __('rate_session.already_reviewed_desc_1') }}
                        <a href="{{ route('patient.appointments.review.edit', $appointment->id) }}"
                            class="underline font-medium hover:text-yellow-900">
                            {{ __('rate_session.edit_link') }}
                        </a>
                        {{ __('rate_session.already_reviewed_desc_2') }}
                        <a href="{{ route('patient.appointments.index') }}"
                            class="underline font-medium hover:text-yellow-900">
                            {{ __('rate_session.back_link') }}
                        </a>.
                    </p>
                </div>
            @else
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Definisi Pesan Error dari Lang PHP ke JS
                const messages = {
                    ratingRequired: "{{ __('rate_session.error_rating_required') }}",
                    charLimit: "{{ __('rate_session.error_char_limit') }}",
                    submitting: "{{ __('rate_session.btn_submitting') }}"
                };
                ...
                if (form) {
                    form.addEventListener('submit', function(e) {
                        ...
                        // Cek rating, kalau pakai component x-star-rating pastikan value tersimpan di input hidden atau sejenisnya
                        // Jika component merender input radio, kita cari yang :checked
                        let ratingValue = 0;

                        // Logic pengecekan rating (sesuaikan dengan component x-star-rating kamu)
                        // Jika x-star-rating menggunakan input hidden dengan name="rating"
                        if (ratingInput) {
                            ratingValue = ratingInput.value;
                        } else {
                            // Fallback jika pakai radio button
