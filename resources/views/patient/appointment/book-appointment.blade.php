    ...
    <div class="flex flex-col flex-1 gap-6">
        ...
        <div class="bg-white p-6 rounded-xl border border-primary shadow-sm">
            ...
            <div class="flex items-center gap-4 p-4 bg-blue-50 rounded-lg">
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-500">Fee: Rp {{ number_format($psychologists[0]->consultation_fee, 0, ',', '.') }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div x-show="loadingDates" class="text-center py-4">
                ...
                <p class="text-sm text-gray-500 mt-2">Loading available dates...</p>
            </div>
            ...
            <div x-show="availableDays.length > 0" class="mt-3">
                <p class="text-sm text-gray-600">
                    <span class="font-medium">Available on:</span>
            ...
            <p x-show="availableDays.length === 0" class="text-gray-500">
                No available days found for this psychologist.
            </p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div x-show="!loadingTimes">
            <div x-show="!selectedDate" class="text-center py-6">
                ...
                <p class="text-sm text-gray-400 mt-1">Please select a date first</p>
            </div>
        ...
        @else
        <!-- FLOW 2 Lasung Ke Book Appointment -->
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            ...
            <p class="text-sm text-gray-500">Select a date to continue</p>
        </div>
        ...
        <!-- PSYCHOLOGIST CARD -->
        <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            <h2 class="font-semibold mb-4 text-lg">3. Available Psychologists</h2>
            <div x-show="loadingPsychologists" class="text-center py-4">
                <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
                <p class="text-sm text-gray-500 mt-2">Loading available psychologists...</p>
            </div>
            <div x-show="!loadingPsychologists">
                ...
                <p x-show="selectedDate && selectedTime && availablePsychologists.length === 0" class="text-gray-500">
                    No psychologists available for the selected date and time.
                </p>
...
function appointmentFlow() {
    return {
        ...
        selectedPsychologist: @if($isSpecific && count($psychologists) > 0) {{ $psychologists[0]->id }} @else null @endif,
        availableDays: [],
        availableDates: [],
        availableTimes: [],
        loadingDates: false,
        ...
        async fetchDates() {
            ...
            try {
                const daysResponse = await fetch(`/patient/psychologist/${this.selectedPsychologist}/available-days`);
                this.availableDays = await daysResponse.json();

                const datesResponse = await fetch(`/patient/psychologist/${this.selectedPsychologist}/available-dates`);
        ...
        async fetchTimes() {
            ...
            try {
                const response = await fetch(`/patient/psychologist/${this.selectedPsychologist}/available-times?date=${this.selectedDate}`);
        ...
        async fetchAvailablePsychologists() {
            ...
            try {
                const response = await fetch(`/patient/available-psychologists?date=${this.selectedDate}&time=${this.selectedTime}`);
        ...
        submitForm() {
            if (!this.selectedDate || !this.selectedTime || !this.selectedPsychologist) {
                alert('Please complete all fields: Date, Time, and Psychologist');
                return;
            }
            ...
            document.getElementById('hiddenConsultationFee').value = this.getConsultationFee(this.selectedPsychologist);
