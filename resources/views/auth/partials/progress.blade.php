            ...
            @foreach ($steps as $step => $label)
                <div class="flex items-center {{ $step < count($steps) ? 'flex-1' : '' }}">
                    <div class="flex flex-col items-center relative z-10">
                        <div
                            class="w-12 h-12 rounded-full flex items-center justify-center mb-2 relative
                            {{ $step < $currentStep
                                ? 'bg-[#009C8F] text-white'
                                : ($step == $currentStep
                                    ? 'bg-[#009C8F] text-white ring-4 ring-[#009C8F] ring-opacity-30 shadow-md'
                        ...
                        <span
                            class="text-sm font-medium {{ $step <= $currentStep ? 'text-[#009C8F]' : 'text-gray-500' }}">
                            {{ $label }}
                        </span>
                    </div>
                    ...
                    @if ($step < count($steps))
                        <div class="flex-1 relative">
                            <div
                                class="absolute top-1/2 left-0 right-0 h-1 bg-gray-200 transform -translate-y-1/2 rounded">
                            </div>
                            @if ($step < $currentStep)
                                <div
                                    class="absolute top-1/2 left-0 right-0 h-1 bg-[#009C8F] transform -translate-y-1/2 rounded">
                                </div>
